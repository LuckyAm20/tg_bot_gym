...
def add_workout_plan(message):
    ...
    if student_id is not None:
        if is_valid_student_id_for_trainer(user_id, student_id):
            ...
            bot.register_next_step_handler(message, process_workout_date_for_plan, student_id, student_username)
        else:
...
def process_workout_date_for_plan(message, student_id, student_username):
    ...
    try:
        ...
        bot.register_next_step_handler(message, save_workout_plan, student_id, workout_date, student_username)
    except ValueError:
