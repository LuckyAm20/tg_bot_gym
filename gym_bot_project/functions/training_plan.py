...
def workout_plan_callback(call, bot):
    user_id = call.from_user.id

    if call.data == "week":
        watch_workout_plan(call, bot)
    elif call.data == "month":
        view_workout_plans_for_month(bot, user_id)
    elif call.data == "year":
        view_workout_plans_for_year(bot, user_id)
    elif call.data == "custom":
        choose_custom_date(bot, user_id)


def choose_custom_date(bot, user_id):
...
def custom_date_callback(call, bot):
    user_id = call.from_user.id
...
def view_workout_plans_for_month(bot, user_id):
    ...
    view_workout_plans(bot, user_id, start_of_prev_month, end_of_prev_month)


def view_workout_plans_for_year(bot, user_id):
    today = datetime.datetime.now().date()
    current_year = today.year
    start_of_prev_year = datetime.date(current_year - 1, 1, 1)
    end_of_prev_year = datetime.date(current_year - 1, 12, 31)
    view_workout_plans(bot, user_id, start_of_prev_year, end_of_prev_year)
